...
for i,l in enumerate(L):
    ...
    for file_name in lista_file:
        ...
        try:
            ...
            if file_name.split("_")[2].split(".")[0][1:] == "Tc":
                T = T_c
        all_data[i].append([T,m,E/(l**2)])

#READ TH DATA
m_th = [[],[]]
with open("M_T_exact.txt") as f:
    reader = csv.reader(f)
    for r in reader:
        m_th[0].append(r[0])
        m_th[1].append(r[1])
...
for i,l in enumerate(L):
    ...
    plots.append(p)
p, = plt.plot(m_th[0],m_th[1],linewidth=2,label="theorical")
plots.append(p)
...
#PLOT ENERGY
plots = []
p, = plt.plot(E_th[0],E_th[1],"g",linewidth=2,label="theorical")
plots.append(p)
for i,l in enumerate(L):
    data_x = []
    data_y = []
    ...
    data_x, data_y = zip(*sorted(zip(data_x,data_y)))
    #p, = plt.plot(data_x,data_y,linestyle="-",linewidth=2,marker="s",markersize=3,label="L={}".format(l))
    p, = plt.plot(data_x,data_y,"kd",markersize=5,label="L={}".format(l))
    plots.append(p)
...
#plt.ylabel(r"$\mathbf{m_{abs} L^{\frac{\beta}{\nu}}}$",fontsize=15)
plt.xlabel(r"$\mathbf{T}$",fontsize=15)
plt.ylabel(r"$\mathbf{\varepsilon}$",fontsize=15)
plt.legend(handles=plots,ncol=2,loc="best")
plt.show()
